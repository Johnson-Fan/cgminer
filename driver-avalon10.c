...
struct avalon10_dev_description avalon10_dev_table[] = {
	{
		...
		AVA10_DEFAULT_VOLTAGE_LEVEL,
		{
			...
			AVA10_DEFAULT_FREQUENCY_0M,
			AVA10_DEFAULT_FREQUENCY_525M,
			AVA10_DEFAULT_FREQUENCY_575M
		}
	}
};
...
static int decode_pkg(struct cgpu_info *avalon10, struct avalon10_ret *ar, int modular_id)
{
	...
	case AVA10_P_NONCE:
		...
		if (!nonce_valid) {
			applog(LOG_NOTICE, "%s-%d-%d: nonce invalid %x",
							avalon10->drv->name, avalon10->device_id, modular_id, tmp);

			info->nonce_get_error_count[modular_id]++;
			break;
		}
		...
		job_id[0] = ar->data[12];
		job_id[1] = ar->data[13];
		pool_no = (ar->data[14] | (ar->data[15] << 8));

		memcpy(&tmp, ar->data + 16, 4);
		info->read_write_error_count[modular_id] = be32toh(tmp);

		memcpy(&tmp, ar->data + 20, 4);
		info->produce_nonce_count[modular_id] = be32toh(tmp);
		...
		tmp = be32toh(tmp);
		info->ecc2_mm[modular_id] = (tmp >> 16) & 0xffff;
		info->ecc2_asic[modular_id] = tmp & 0xffff;
		...
		if (pool_no >= total_pools || pool_no < 0) {
			applog(LOG_NOTICE, "%s-%d-%d: Wrong pool_no %d",
		...
		if (!submit_nonce2_nonce(thr, pool, real_pool, nonce2, nonce, ntime, micro_job_id)) {
			info->hw_works_i[modular_id][miner_id]++;
			info->chip_error_work[modular_id][miner_id][asic_id]++;
		} else {
...
static void avalon10_set_factory_pll_info(struct cgpu_info *avalon10, int addr, int val)
{
	struct avalon10_pkg send_pkg;
	...
	/* Package the data */
	avalon10_init_pkg(&send_pkg, AVA10_P_SET_FAC_PLL, 1, 1);
	if (addr == AVA10_MODULE_BROADCAST)
		avalon10_send_bc_pkgs(avalon10, &send_pkg);
	else
		avalon10_iic_xfer_pkg(avalon10, addr, &send_pkg, NULL);
...
static void avalon10_set_factory_volt_info(struct cgpu_info *avalon10, int addr, int val)
{
	struct avalon10_pkg send_pkg;
	uint32_t tmp;
	...
	/* Package the data */
	avalon10_init_pkg(&send_pkg, AVA10_P_SET_FAC_VOLT, 1, 1);
	if (addr == AVA10_MODULE_BROADCAST)
		avalon10_send_bc_pkgs(avalon10, &send_pkg);
	else
		avalon10_iic_xfer_pkg(avalon10, addr, &send_pkg, NULL);
...
static void detect_modules(struct cgpu_info *avalon10)
{
	...
	for (i = 1; i < AVA10_DEFAULT_MODULARS + 1; i++) {
		...
		for (j = 0; j < info->miner_count[i]; j++) {
			...
			memset(info->chip_error_work[i][j], 0, sizeof(uint64_t) * info->asic_count[i]);
		...
		info->produce_nonce_count[i] = 0;
		info->get_nonce_count[i] = 0;
		info->read_write_error_count[i] = 0;
		info->ecc2_mm[i] = 0;
		info->ecc2_asic[i] = 0;
		info->nonce_get_error_count[i] = 0;
...
static struct api_data *avalon10_api_stats(struct cgpu_info *avalon10)
{
	...
	for (i = 1; i < AVA10_DEFAULT_MODULARS; i++) {
		...
		for (j = 0; j < info->miner_count[i]; j++) {
			...
			strcat(statbuf, buf);
			for (k = 0; k < info->asic_count[i]; k++) {
				sprintf(buf, "%"PRIu64" ", info->chip_error_work[i][j][k]);
				strcat(statbuf, buf);
			}
		...
		sprintf(buf, " ECC2_MM[%u]", info->ecc2_mm[i]);
		strcat(statbuf, buf);

		sprintf(buf, " ECC2_ASIC[%u]", info->ecc2_asic[i]);
		strcat(statbuf, buf);

		sprintf(buf, " NONCE_INVALID[%u]", info->nonce_get_error_count[i]);
		strcat(statbuf, buf);

		sprintf(buf, " ReadWriteError[%u]", info->read_write_error_count[i]);
		strcat(statbuf, buf);

		sprintf(buf, " ProduceNonceCount[%u]", info->produce_nonce_count[i]);
		strcat(statbuf, buf);

		sprintf(buf, " GetNonceCount[%u]", info->get_nonce_count[i]);
		strcat(statbuf, buf);

		{
			...
			for (j = 0; j < info->miner_count[i]; j++) {
				for (k = 0; k < info->asic_count[i]; k++)
					sum += info->chip_matching_work[i][j][k];
			}

			sprintf(buf, " MW_SUM[%u]", sum);
			strcat(statbuf, buf);

			sum = 0;
			for (j = 0; j < info->miner_count[i]; j++) {
				for (k = 0; k < info->asic_count[i]; k++)
					sum += info->chip_error_work[i][j][k];
			}

			sprintf(buf, " MWE_SUM[%u]", sum);
			strcat(statbuf, buf);
		}
...
char *set_avalon10_factory_pll_info(struct cgpu_info *avalon10, char *arg)
{
	struct avalon10_info *info = avalon10->device_data;
	int val, addr, i;
	...
	if (addr >= AVA10_DEFAULT_MODULARS) {
		applog(LOG_ERR, "invalid modular index: %d, valid range 0-%d", addr, (AVA10_DEFAULT_MODULARS - 1));
		return "Invalid modular index to set_avalon10_factory_pll_info";
	}
	...
	if (!addr) {
		for (i = 1; i < AVA10_DEFAULT_MODULARS; i++) {
			...
			avalon10_set_factory_pll_info(avalon10, i, val);
		}
	} else {
			avalon10_set_factory_pll_info(avalon10, addr, val);
	}

	applog(LOG_NOTICE, "%s-%d: Update factory-pll info %d-%d",
			avalon10->drv->name, avalon10->device_id, val, addr);

	return NULL;
}

char *set_avalon10_factory_volt_info(struct cgpu_info *avalon10, char *arg)
{
	struct avalon10_info *info = avalon10->device_data;
	int val, addr, i;
	...
	if (addr >= AVA10_DEFAULT_MODULARS) {
		applog(LOG_ERR, "invalid modular index: %d, valid range 0-%d", addr, (AVA10_DEFAULT_MODULARS - 1));
		return "Invalid modular index to set_avalon10_factory_volt_info";
	}
	...
	if (!addr) {
		for (i = 1; i < AVA10_DEFAULT_MODULARS; i++) {
			...
			avalon10_set_factory_volt_info(avalon10, i, val);
		}
	} else {
			avalon10_set_factory_volt_info(avalon10, addr, val);
	}

	applog(LOG_NOTICE, "%s-%d: Update factory-volt info %d-%d",
			avalon10->drv->name, avalon10->device_id, val, addr);
...
static char *avalon10_set_device(struct cgpu_info *avalon10, char *option, char *setting, char *replybuf)
{
	...
	if (strcasecmp(option, "factory-pll") == 0) {
		if (!setting || !*setting) {
			sprintf(replybuf, "missing factory-pll info");
			return replybuf;
		}

		return set_avalon10_factory_pll_info(avalon10, setting);
	}
